...
public partial class PersonaIdentitePage : ContentPage
{
	public PersonaIdentitePage()
	{
		InitializeComponent();
	}
  ...
  private async void OnNextButtonClicked(object sender, TappedEventArgs e)
  {
    try
    {
      await App.Current.MainPage.Navigation.PushAsync(new VerifyMangerDetailsPage());
    }
    catch (Exception ex)
    {
      System.Diagnostics.Debug.WriteLine($"Navigation Error: {ex.Message}");
